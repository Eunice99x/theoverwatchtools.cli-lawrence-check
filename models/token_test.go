...
func testTokenToOneUserUsingCreatedByUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCreatedByUser(ctx, tx, false, (*[]*Token)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTokenToOneSetOpUserUsingCreatedByUser(t *testing.T) {
	var err error
...
var (
	tokenDBTypes = map[string]string{`ID`: `int`, `Key`: `varchar`, `CreatedAt`: `timestamp`, `Revoked`: `tinyint`, `Expired`: `tinyint`, `CreatedBy`: `int`, `ExpiresAt`: `timestamp`}
	_            = bytes.MinRead
)
