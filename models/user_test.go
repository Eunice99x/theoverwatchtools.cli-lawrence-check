...
func testUserToManyLastUpdatedByUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.LastUpdatedBy, a.ID)
	queries.Assign(&c.LastUpdatedBy, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.LastUpdatedByUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.LastUpdatedBy, b.LastUpdatedBy) {
			bFound = true
		}
		if queries.Equal(v.LastUpdatedBy, c.LastUpdatedBy) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadLastUpdatedByUsers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.LastUpdatedByUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.LastUpdatedByUsers = nil
	if err = a.L.LoadLastUpdatedByUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.LastUpdatedByUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpLastUpdatedByUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddLastUpdatedByUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.LastUpdatedBy) {
			t.Error("foreign key was wrong value", a.ID, first.LastUpdatedBy)
		}
		if !queries.Equal(a.ID, second.LastUpdatedBy) {
			t.Error("foreign key was wrong value", a.ID, second.LastUpdatedBy)
		}

		if first.R.LastUpdatedByUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.LastUpdatedByUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.LastUpdatedByUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.LastUpdatedByUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.LastUpdatedByUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpLastUpdatedByUsers(t *testing.T) {
	var err error
	...
	err = a.SetLastUpdatedByUsers(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetLastUpdatedByUsers(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.LastUpdatedBy) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.LastUpdatedBy) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.LastUpdatedBy) {
		t.Error("foreign key was wrong value", a.ID, d.LastUpdatedBy)
	}
	if !queries.Equal(a.ID, e.LastUpdatedBy) {
		t.Error("foreign key was wrong value", a.ID, e.LastUpdatedBy)
	}

	if b.R.LastUpdatedByUser != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.LastUpdatedByUser != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.LastUpdatedByUser != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.LastUpdatedByUser != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}

	if a.R.LastUpdatedByUsers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.LastUpdatedByUsers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpLastUpdatedByUsers(t *testing.T) {
	var err error
	...
	err = a.AddLastUpdatedByUsers(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveLastUpdatedByUsers(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.LastUpdatedByUsers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.LastUpdatedByUsers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.LastUpdatedByUsers[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testUserToOneCategoryUsingCategoryTypeRef(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCategoryTypeRef(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToOneUserUsingLastUpdatedByUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.LastUpdatedBy, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.LastUpdatedByUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&local}
	if err = local.L.LoadLastUpdatedByUser(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.LastUpdatedByUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.LastUpdatedByUser = nil
	if err = local.L.LoadLastUpdatedByUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.LastUpdatedByUser == nil {
		t.Error("struct should have been eager loaded")
...
func testUserToOneSetOpCategoryUsingCategoryTypeRef(t *testing.T) {
	var err error
	...
	for i, x := range []*Category{&b, &c} {
		...
		if x.R.CategoryTypeRefUsers[0] != &a {
...
func testUserToOneSetOpUserUsingLastUpdatedByUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetLastUpdatedByUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.LastUpdatedByUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.LastUpdatedByUsers[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.LastUpdatedBy, x.ID) {
			t.Error("foreign key was wrong value", a.LastUpdatedBy)
		}

		zero := reflect.Zero(reflect.TypeOf(a.LastUpdatedBy))
		reflect.Indirect(reflect.ValueOf(&a.LastUpdatedBy)).Set(zero)
		...
		if !queries.Equal(a.LastUpdatedBy, x.ID) {
			t.Error("foreign key was wrong value", a.LastUpdatedBy, x.ID)
		}
	}
}

func testUserToOneRemoveOpUserUsingLastUpdatedByUser(t *testing.T) {
	var err error
	...
	if err = a.SetLastUpdatedByUser(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveLastUpdatedByUser(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.LastUpdatedByUser().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.LastUpdatedByUser != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.LastUpdatedBy) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.LastUpdatedByUsers) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `Firstname`: `varchar`, `Lastname`: `varchar`, `Email`: `varchar`, `Password`: `varchar`, `CategoryTypeRefID`: `int`, `CreatedBy`: `int`, `LastUpdatedBy`: `int`, `CreatedAt`: `timestamp`, `LastUpdatedAt`: `timestamp`, `IsActive`: `tinyint`, `ResetToken`: `varchar`, `Address`: `varchar`, `Birthday`: `date`, `Gender`: `char`}
	_           = bytes.MinRead
)
